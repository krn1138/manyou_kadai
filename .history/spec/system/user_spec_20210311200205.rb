...
describe 'ユーザー登録' do
    def login(usertwo)
        visit new_session_path
        fill_in 'session[email]', with: usertwo.email
        fill_in 'session[password]', with: usertwo.password
        click_button 'Log in'
        puts 'user logged in.'
    end
    def admin_login(admin_user)
    ...
    context 'ユーザが新規登録した場合' do
      it '新規登録ができること' do
        # login(user)
        visit new_user_path
        fill_in 'user[name]', with: usertwo.name
        fill_in 'user[email]', with: usertwo.email
        fill_in 'user[password]', with: usertwo.password
        fill_in 'user[password_confirmation]', with: usertwo.password_confirmation
        choose 'user_admin_なし'
        click_button 'Create my account'
      end
    end
